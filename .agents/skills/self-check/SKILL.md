---
name: self-check
description: Self-check for agent-config-doctor's own project. Use when modifying agent-config-doctor itself.
...
Self-check for agent-config-doctor's own project. Runs in two phases:

1. **Structural Check** — runs the agent-config-doctor:check procedure against this project
...
1. Read the following files to understand the project's design intent and conventions:
   - `skills/check/SKILL.md`, `skills/check/project.md`, `skills/check/reference.md`
...
- Check that `README.md` and `CONTRIBUTING.md` accurately reflect the changes in the diff.
- If the diff adds/removes/changes features, check items, sections, or behavior: verify the README documents them.
- If the diff modifies check sections or output format: verify the "What it checks" table in README is updated.
